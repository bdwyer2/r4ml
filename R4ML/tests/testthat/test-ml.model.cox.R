...
test_that("r4ml.coxph accuracy", {
  ...
  if (any(c(res1, res2))) {
      message("Since for comparing internally we created a sorted structure and
              there could be the bug in the different OS sort.so we are just printing the error msg without error status")
